...
async function checkEmail() {
  ...
  if (effectiveStep === "password") {
    renderUserChip("user-chip-pw", STATE.name);
...
document
  .getElementById("btn-diff-email-pw")
  .addEventListener("click", resetToEmail);
document
  .getElementById("btn-diff-email-first")
  .addEventListener("click", resetToEmail);
...
async function sendActivationOtp() {
  ...
  if (!ok) {
    ...
    if (message && message.toLowerCase().includes("already activated")) {
      markEmailAsActivated(STATE.email);
      showAlert(
        "Your account is already activated. Please log in with your password.",
      );
      setTimeout(() => {
        renderUserChip("user-chip-pw", STATE.name);
        showStep("step-password");
...
document
  ...
  .addEventListener("click", sendActivationOtp);
...
document
  .getElementById("btn-forgot-send")
...
async function verifyOtp() {
  ...
  const { ok, data, message } = await apiVerifyOtp(
    STATE.email,
    otp,
    STATE.purpose,
  );
  ...
  if (!ok)
    return showAlert(message || "Invalid OTP. Please check and try again.");
  ...
  if (STATE.purpose === "reset_password") {
    ...
    document.getElementById("btn-setpw-label").textContent =
...
async function setPassword() {
  ...
  if (!STATE.tempToken)
    return showAlert("Verification token missing. Please start over.");
