...
import { hardDeleteCascade, hardDeleteManyCascade } from '../utils/hardDelete.js';
import { storeDataUri } from '../utils/gridfs.js';
import { createStudent, ensureUser, ensureEmailAvailable, live, logCoHodActivity, mapStudent, mapTeacher, normalizeSubjectPayload, syncSubjectSchedule, resolveSubjectForSlot, timesOverlap, updateStudentAndSyncParent, validateMobileNumber, validateImageDataUri, findAttendanceTimeConflict, validatePdfDataUri, validateAttendanceDateIsNotInTheFuture, validateLectureDuration } from '../controllers/common.js';
...
function normalizeLabRollRange(slot) {
  ...
  const labRollStart = String(slot.labRollStart || slot.labStartRoll || slot.rollStart || '').trim();
  const labRollEnd = String(slot.labRollEnd || slot.labEndRoll || slot.rollEnd || '').trim();
  if (type !== 'Lab') return { labRollStart: '', labRollEnd: '' };
  if (!labRollStart || !labRollEnd) {
    const err = new Error('Lab slots require starting and ending student roll numbers.');
    err.status = 400;
    throw err;
  }
...
function scheduleSlotsConflict(a, b) {
  ...
  if (a.type === 'Lab' && b.type === 'Lab') {
    return rollRangesOverlap(a.labRollStart, a.labRollEnd, b.labRollStart, b.labRollEnd);
  }
  return true;
}
...
async function ensureLabRollsExist({ college, department, course, semester, labRollStart, labRollEnd }) {
  ...
  }).select('roll rollNo rollNumber').lean();
  const rolls = new Set(docs.flatMap(s => [s.roll, s.rollNo, s.rollNumber].filter(Boolean).map(v => String(v).trim())));
  if (!rolls.has(String(labRollStart).trim()) || !rolls.has(String(labRollEnd).trim())) {
    const err = new Error('Lab start and end roll numbers must be selected from existing students in this course and semester.');
    err.status = 400;
    throw err;
  }
}
...
async function buildHodProfile(user) {
  const [studentCourses, teacherCourses, courseDocs, departmentDoc] = await Promise.all([
  ...
  return { id: user.id, name: user.name, email: user.email, phone: user.phone || '', role: user.role,
    ...
    college: user.college, departmentId: user.department,
    department: departmentDoc?.name || 'Department', designation: user.designation || 'HOD',
...
router.get('/students', asyncHandler(async (req, res) => {
  ...
  filter.status = req.query.status === 'Removed' ? 'Removed' : { $ne: 'Removed' };
...
router.post('/students', asyncHandler(async (req, res) => {
  ok(res, { student: mapStudent(await createStudent(req.body, req.user, { requireParentEmail: false, requireStudentEmail: true })) }, 'Student saved.');
}));
...
router.delete('/students/:id', asyncHandler(async (req, res) => {
  const student = await Student.findOne({ _id: req.params.id, college: req.user.college, department: req.user.department });
  if (!student) return fail(res, 404, 'Student not found.');
...
router.post('/students/remove', asyncHandler(async (req, res) => {
  const ids = req.body.ids || [];
  if (!ids.length) return fail(res, 400, 'No student IDs provided.');
  await Student.updateMany(
    { _id: { $in: ids }, college: req.user.college, department: req.user.department },
...
router.post('/students/recover', asyncHandler(async (req, res) => {
  const ids = req.body.ids || [];
  if (!ids.length) return fail(res, 400, 'No student IDs provided.');
  await Student.updateMany(
    { _id: { $in: ids }, college: req.user.college, department: req.user.department, status: 'Removed' },
...
router.delete('/students', asyncHandler(async (req, res) => {
  const ids = req.body.ids || [];
  if (!ids.length) return fail(res, 400, 'No student IDs provided.');
  const count = await hardDeleteManyCascade(Student, { _id: { $in: ids }, college: req.user.college, department: req.user.department }, req.user);
...
router.post('/import/students', asyncHandler(async (req, res) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    try {
      docs.push(await createStudent({ ...row, course: row.course || req.body.course, sem: row.sem || req.body.sem }, req.user, { requireParentEmail: false }));
    } catch (e) {
      errors.push({ row: i + 1, name: row.name || '(no name)', message: e.message });
    }
  }
  ok(res, { imported: docs.length, students: docs.map(mapStudent), errors },
    errors.length ? `${docs.length} imported, ${errors.length} failed — see details.` : `${docs.length} students imported.`);
}));
...
router.get('/teachers/:id/detail', asyncHandler(async (req, res) => {
  ...
  const [schedule, attendance, syllabus] = await Promise.all([
    Schedule.find({ ...scope(req), teacher: teacher._id }).sort({ day: 1, startTime: 1 }).lean(),
    Attendance.find({ ...scope(req), teacher: teacher._id })
      .select('date course semester subject subjectName division time type status topic isProxy')
      .lean(),
  ...
  const sessionList = Array.from(sessions.values()).sort((a, b) => new Date(b.date) - new Date(a.date));
  ok(res, {
...
router.put('/teachers/:id', asyncHandler(async (req, res) => {
  ...
  if (body.email) {
    try { body.email = await ensureEmailAvailable(body.email, { excludeUserId: req.params.id }); }
...
router.delete('/teachers/:id', asyncHandler(async (req, res) => {
  const teacher = await User.findOne({ _id: req.params.id, college: req.user.college, department: req.user.department });
  if (!teacher) return fail(res, 404, 'Teacher not found.');
...
router.delete('/teachers', asyncHandler(async (req, res) => {
  ...
  const count = await hardDeleteManyCascade(User, { _id: { $in: ids }, college: req.user.college, department: req.user.department }, req.user);
...
router.post('/import/teachers', asyncHandler(async (req, res) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    try {
      docs.push(await ensureUser({ ...row, email: row.email || `t_${Date.now()}_${Math.random().toString(36).slice(2)}@sal.local`, role: 'teacher', college: req.user.college, department: req.user.department }, req.user.id));
    } catch (e) {
      errors.push({ row: i + 1, name: row.name || '(no name)', message: e.message });
    }
  }
  ok(res, { imported: docs.length, teachers: docs.map(mapTeacher), errors },
    errors.length ? `${docs.length} imported, ${errors.length} failed — see details.` : `${docs.length} teachers imported.`);
}));
...
router.get('/subjects', asyncHandler(async (req, res) => {
  ...
  for (const s of subjects) {
    ...
    const course = s.course; const sem = String(s.semester || s.sem || 1);
    if (!subjectsMap[course]) subjectsMap[course] = {};
    if (!subjectsMap[course][sem]) subjectsMap[course][sem] = [];
    subjectsMap[course][sem].push({ id: s._id, _id: s._id, name: s.name, code: s.code });
  }
...
router.post('/subjects', asyncHandler(async (req, res) => {
  ...
  const body = await normalizeSubjectPayload({ ...req.body, departmentId: req.user.department }, req.user);
  ...
  if (dup) return fail(res, 409, `"${body.name}" already exists for this course/semester.`);
  ...
  await logCoHodActivity(req.user, `Added subject "${subject.name}" to ${subject.course} · Sem ${subject.semester}`);
  ok(res, { subject }, 'Subject added.');
}));
router.put('/subjects/:id', asyncHandler(async (req, res) => {
  const existing = await Subject.findOne({ _id: req.params.id, college: req.user.college, department: req.user.department });
  if (!existing) return fail(res, 404, 'Subject not found in your department.');
  const body = await normalizeSubjectPayload({
    course: existing.course, semester: existing.semester, ...req.body, departmentId: req.user.department
  }, req.user);
  const subject = await Subject.findOneAndUpdate(
    { _id: req.params.id, college: req.user.college, department: req.user.department },
    { ...body, updatedBy: req.user.id }, { new: true }
  );
  if (subject) await syncSubjectSchedule(subject, req.user);
  if (!subject) return fail(res, 404, 'Subject not found.');
  await logCoHodActivity(req.user, `Updated subject "${subject.name}" (${subject.course} · Sem ${subject.semester})`);
  ok(res, { subject }, 'Subject updated.');
}));
router.delete('/subjects/:id', asyncHandler(async (req, res) => {
  const subject = await Subject.findOne({ _id: req.params.id, college: req.user.college, department: req.user.department });
  if (!subject) return fail(res, 404, 'Subject not found in your department.');
  const { name, course, semester } = subject;
  await hardDeleteCascade(subject, req.user);
  await logCoHodActivity(req.user, `Removed subject "${name}" (${course} · Sem ${semester})`);
...
router.post('/attendance', asyncHandler(async (req, res) => {
  ...
  if (attendanceStartTime && attendanceEndTime) {
    const durationCheck = validateLectureDuration(attendanceStartTime, attendanceEndTime);
  ...
  const timeConflict = await findAttendanceTimeConflict({
    college: req.user.college, department: req.user.department, course, semester,
    division, date: sessionDate, startTime: attendanceStartTime || '', endTime: attendanceEndTime || '',
...
router.get('/attendance/history', asyncHandler(async (req, res) => {
  ...
  const docs = await Attendance.find(filter).populate('teacher', 'name')
    .select('date course semester subject subjectName division time type status topic isProxy teacher').lean();
  ...
  for (const d of docs) {
    ...
    if (!sessions.has(key)) {
      sessions.set(key, {
        ...
        topic: d.topic || '', isProxy: !!d.isProxy,
...
router.get('/attendance/session/:sessionKey', asyncHandler(async (req, res) => {
  ...
  ok(res, {
    ...
    meta: {
      ...
      topic: records[0].topic || '', isProxy: !!records[0].isProxy,
...
router.delete('/marks/:id', asyncHandler(async (req, res) => {
  ...
  if (!mark) return fail(res, 404, 'Mark not found.'); await hardDeleteCascade(mark, req.user); ok(res, {}, 'Mark permanently deleted.');
}));
  ...
  for (const s of usableSlots) {
    ...
    try {
      ...
      await ensureLabRollsExist({
        college: req.user.college, department: req.user.department,
        course, semester: semesterNum, ...labRange,
      });
    } catch (e) {
      return fail(res, e.status || 400, `"${subject.name}" (${s.day}): ${e.message}`);
    }
...
router.post('/schedule/slot', asyncHandler(async (req, res) => {
  ...
  let labRange;
  try {
    labRange = normalizeLabRollRange({ ...req.body, type: type || subject.type || 'Lecture' });
    await ensureLabRollsExist({
      college: req.user.college, department: req.user.department,
      course, semester, ...labRange,
    });
  } catch (e) {
    return fail(res, e.status || 400, e.message);
  }
  ...
  const conflict = await findConflictingSlotExcluding({
    college: req.user.college, department: req.user.department, course, semester,
    day, startTime, endTime, teacher: teacherDoc._id, excludeIds: [],
    type: type || subject.type || 'Lecture', ...labRange,
  });
...
router.put('/schedule/:id', asyncHandler(async (req, res) => {
  ...
  try {
    const labRange = normalizeLabRollRange({ ...existing.toObject(), ...body });
    await ensureLabRollsExist({
      college: req.user.college, department: req.user.department,
      course, semester: semesterNum, ...labRange,
    });
  ...
  const conflict = await findConflictingSlotExcluding({
    college: req.user.college, department: req.user.department, course, semester: semesterNum,
    day, startTime, endTime, teacher: teacherId, excludeIds: [existing._id],
    type: body.type || existing.type || 'Lecture',
    labRollStart: body.labRollStart, labRollEnd: body.labRollEnd,
  });
  if (conflict) return fail(res, 409, conflict.message);
...
async function findConflictingSlotExcluding({ college, department, course, semester, day, startTime, endTime, teacher, excludeIds, type, labRollStart, labRollEnd }) {
  ...
  for (const slot of classSlots) {
    if (scheduleSlotsConflict({ startTime, endTime, type, labRollStart, labRollEnd }, slot)) {
...
router.post('/announcements', asyncHandler(async (req, res) => {
  ...
  if (req.body.attachment) {
    try { attachment = await storeDataUri(validatePdfDataUri(req.body.attachment), `announcement-${Date.now()}`); }
    catch (e) { return fail(res, e.status || 400, e.message); }
  }
  const ann = await Notice.create({
    ...
    targetTeacherIds: Array.isArray(req.body.targetTeacherIds) ? req.body.targetTeacherIds : [],
...
router.delete('/announcements/:id', asyncHandler(async (req, res) => {
  ...
  if (!notice) return fail(res, 404, 'Announcement not found.'); await hardDeleteCascade(notice, req.user); ok(res, {}, 'Announcement permanently deleted.');
}));
...
router.get('/co-hod-history', asyncHandler(async (req, res) => {
  const entries = await CoHodActivity.find({ college: req.user.college, department: req.user.department })
