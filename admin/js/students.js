...
const Students = {
  ...
  async add() {
    ...
    if (!dept) { UI.toast('Department is required.', 'error'); return; }
    if (!course) { UI.toast('Course is required.', 'error'); return; }
  ...
  async del(id, name) {
    UI.confirmDelete({ itemLabel: 'student', name, onConfirm: async () => {
      try {
