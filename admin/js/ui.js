...
function avatarCell(person) {
  ...
  if (photo) {
    return `<img class="av" src="${photo}" alt="${name}" style="width:32px;height:32px;border-radius:50%;object-fit:cover">`;
  }
  return `<div class="av">${UI.initials(name)}</div>`;
}
...
// simple confirm()).
UI.confirmDelete = function ({ title = 'Delete', name, itemLabel = 'record', onConfirm }) {
