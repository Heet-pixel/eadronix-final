...
async function salFetch(m, p, b = null, retry = false) {
  ...
  const url = "/api" + p;
  // GET requests go through OfflineDataCache when it's loaded (see
  // shared/js/offline-data-cache.js): online, it behaves exactly like a
  // normal fetch; offline (or on a network error), it transparently
  // serves back the last successful response for this exact URL instead
  // of failing, so Course/Subject/Student data — and therefore marking
  // attendance — keeps working offline. Writes (POST/PUT/DELETE) are
  // never cached and always go straight to the network, unchanged.
  let r =
    m === "GET" && window.OfflineDataCache
      ? await OfflineDataCache.fetchJson(url, o)
      : await fetch(url, o);
...
const TAPI = {
  ...
  getAttReport: (q = "") =>
    get("/teacher/attendance-report" + (q ? "?" + q : "")),

  // Attendance History (spec item 4): list every lecture, view/edit one.
  getAttHistory: (q = "") =>
    get("/teacher/attendance/history" + (q ? "?" + q : "")),
  getAttSession: (key) =>
    get("/teacher/attendance/session/" + encodeURIComponent(key)),
  editAttSession: (key, records) =>
    put("/teacher/attendance/session/" + encodeURIComponent(key), { records }),
  ...
  getMarksAccess: (course, semester) =>
    get(
      `/teacher/marks/access?course=${encodeURIComponent(course)}&semester=${semester}`,
    ),
  getMarksGrid: (course, semester, examType) =>
    get(
      `/teacher/marks/grid?course=${encodeURIComponent(course)}&semester=${semester}&examType=${encodeURIComponent(examType)}`,
    ),
  saveMarksGrid: (records) => post("/teacher/marks/grid-upsert", { records }),
  publishMarks: (course, semester, examType, publish, subjectIds) =>
    salFetch("PATCH", "/teacher/marks/publish", {
      course,
      semester,
      examType,
      publish,
