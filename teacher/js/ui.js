...
const UI = {
  ...
  // Used as the onerror handler on every profile <img> across the portal.
  // If the photo URL fails to load (network hiccup, storage issue, etc.)
  // this swaps it for the plain initials avatar instead of leaving a
  // broken image / raw alt-text showing.
  ...
  closeAllModals() {
    document
      .querySelectorAll(".modal.open, .modal-overlay.open")
  ...
  show(id) {
    ...
    document
      ...
      .forEach((b) => b.classList.toggle("active", b.id === "bn-" + id));
    document
      .querySelectorAll(".sn-item")
      .forEach((b) => b.classList.toggle("active", b.id === "sni-" + id));
    window.scrollTo(0, 0);
...
function closeStuModal() {
  UI.closeModal("stuModal");
}
function closeSubjModal() {
  UI.closeModal("subjDetailModal");
}
