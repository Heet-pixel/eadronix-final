...
let stuReports = [],
  tchrReports = [];
let activeStuCourse = null,
  activeStuSem = null,
  activeStuAllSems = false;
let stuDeleteMode = false,
...
let currentStuId = null,
  currentTchrId = null;
let stuEditMode = false,
  tchrEditMode = false;
let tchrFilterCourse = "",
  tchrFilterSem = "";
...
function getSubjObjects(course, sem) {
  if (!SUBJECTS[course] || !SUBJECTS[course][sem]) return [];
  return SUBJECTS[course][sem].map((s) =>
    typeof s === "object"
      ? { id: s.id || s._id, name: s.name }
      : { id: null, name: s },
  );
}
...
async function buildStuReports(course, sem, forceFresh = false) {
  ...
  const cohort = allStudents.filter(
    (s) => s.course === course && (!sem || s.sem === Number(sem)),
  );
  stuReports = cohort.map((s) => {
    ...
    const subjects = row.subjects.map((sub) => ({
      subject: sub.name,
...
function invalidateAttReportCache() {
  _attReportCache = {};
}
...
async function apiJson(url, opts) {
  ...
  // of failing, so bootstrap data (HOD profile, courses, students,
  // teachers, subjects, schedule, etc.) — and therefore marking
  // attendance — keeps working offline. Writes are never cached and
  // always go straight to the network, unchanged.
  const res =
    method === "GET" && window.OfflineDataCache
      ? await OfflineDataCache.fetchJson(url, fetchOpts)
