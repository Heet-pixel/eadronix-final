...
async function bootstrap() {
  ...
  try {
    ...
    document.getElementById("sidebarName").textContent =
      currentHOD.name || "Dept Admin";
    const badgeEl = document.getElementById("sidebarBadge");
    if (badgeEl)
      badgeEl.textContent =
        currentHOD.role === "co_hod" ? "Co-Dept Admin" : "Dept Admin";
    const sbAv = document.getElementById("sidebarAvatar");
    if (sbAv) {
      const sbInitial = (currentHOD.name || "H")
        .trim()
        .slice(0, 1)
        .toUpperCase();
      sbAv.innerHTML = currentHOD.avatar
        ? `<img src="${currentHOD.avatar}" alt="" style="width:100%;height:100%;border-radius:50%;object-fit:cover" onerror="avatarFallback(this, '${sbInitial}')">`
        : sbInitial;
    }
    updateProfile();
    loadDashboard();
...
function updateProfile() {
  ...
  if (el("profileInitial")) {
    el("profileInitial").innerHTML = currentHOD.avatar
      ? `<img src="${currentHOD.avatar}" alt="" style="width:100%;height:100%;border-radius:50%;object-fit:cover" onerror="avatarFallback(this, '${ini}')">`
      : ini;
  }
...
const SECTION_META = {
  ...
  subjects: {
    title: "Subjects",
    sub: "Add and manage subjects for your department's courses & semesters",
  },
  cohodhistory: {
    title: "Co-HOD History",
    sub: "Everything added, changed, or removed in your department, with who did it",
  },
...
function showSection(id) {
  ...
  const fns = {
    ...
    subjects:
      typeof loadSubjectsSection === "function"
        ? loadSubjectsSection
        : undefined,
    cohodhistory:
      typeof loadCoHodHistory === "function" ? loadCoHodHistory : undefined,
  };
  if (fns[id]) fns[id]();
}
