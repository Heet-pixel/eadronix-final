...
async function openStudentModal(id) {
  ...
  try {
    ...
    showToast(
      "Could not refresh latest attendance. Showing last loaded data.",
      true,
    );
  }
  currentStuId = id;
  stuEditMode = false;
  {
    const modalAvatarFallback =
      "https://ui-avatars.com/api/?name=" +
      encodeURIComponent(s.name) +
      "&size=150&background=random";
    const modalAvatarEl = document.getElementById("modalAvatar");
    modalAvatarEl.src = s.avatar || modalAvatarFallback;
    modalAvatarEl.onerror = function () {
      this.onerror = null;
      this.src = modalAvatarFallback;
    };
  }
  document.getElementById("modalName").textContent = s.name;
  document.getElementById("modalMeta").textContent =
    `${s.course} | Semester ${s.sem} | Roll: ${s.roll}`;
  const addressText = [s.address, s.city, s.state, s.pincode]
    .filter(Boolean)
    .join(", ");
  document.getElementById("modalPersonal").innerHTML =
    mField("Full Name", s.name, "name") +
    mField("Gender", s.gender, "gender") +
    mField("Date of Birth", s.dob, "dob") +
    mField("Blood Group", s.bloodGroup, "bloodGroup") +
    mField("Phone", s.phone, "phone") +
    mField("Email", s.email, "email") +
    mField("Address", addressText, "address") +
    mField("City", s.city, "city");
  document.getElementById("modalParent").innerHTML =
    mField("Parent Name", s.parentName, "parentName") +
    mField("Relation", s.parentRelation, "parentRelation") +
    mField("Parent Phone", s.parentPhone, "parentPhone") +
    mField("Parent Email", s.parentEmail, "parentEmail", "email") +
    mFieldSel("Category", s.category, "category", [
      "General",
      "OBC",
      "SC",
      "ST",
  ...
  document.getElementById("modalAcademic").innerHTML =
    mField("Course", s.course, "course") +
    mField("Semester", s.sem, "sem", "number") +
    mField("Roll No", s.roll, "roll") +
    mField("Admission Year", s.admissionYear, "admissionYear", "number") +
    mFieldSel("Status", s.status, "status", ["Active", "Inactive"]);
  // Remove old dynamic sections
  document
    .querySelectorAll("#studentModal .att-dyn-section")
  ...
  let subs = allSubjNames.map((subName) => {
    let ex = rpt
      ? (rpt.subjects || []).find((x) => x.subject === subName)
      : null;
    if (ex) return ex;
  ...
  let pctColor =
    overallPct >= 75
      ...
      : overallPct >= 60
        ...
        : "var(--danger)";
  let attHtml =
    '<div class="modal-section att-dyn-section"><h4>Attendance Overview</h4>';
  ...
  attHtml +=
    '<div class="modal-section"><h4>Subjects &mdash; Tap to View Lectures</h4><div class="subj-list">';
  subs.forEach((sub) => {
    let pctCls =
      sub.pct >= 75
        ? "badge-green"
  ...
  let firstSection = document.querySelector(
    "#studentModal .modal-body .modal-section",
  );
...
async function openLecHist(stuId, subjName) {
  ...
  document.getElementById("lecHistTitle").textContent =
    subjName + " — Lecture History";
  let dates = Object.keys(
    ((savedAttendance[s.course] || {})[s.sem] || {})[subjName] || {},
  );
  let records = [];
  if (dates.length) {
    dates.sort().forEach((date, i) => {
      let code =
        (savedAttendance[s.course][s.sem][subjName][date] || {})[stuId] || "P";
      records.push({
        num: i + 1,
        date,
        status: code === "P" ? "present" : code === "A" ? "absent" : "extra",
      });
    });
  } else {
    let rpt = stuReports.find((r) => String(r.id) === String(stuId));
    let sub = rpt
      ? (rpt.subjects || []).find((x) => x.subject === subjName)
      : null;
    let total = sub ? sub.total : 0,
      attended = sub ? sub.attended : 0;
...
async function openTeacherModal(id) {
  ...
  tchrEditMode = false;
  {
    const tModalAvatarFallback =
      "https://ui-avatars.com/api/?name=" +
      encodeURIComponent(t.name) +
      "&size=150&background=random";
    const tModalAvatarEl = document.getElementById("tModalAvatar");
    tModalAvatarEl.src = t.avatar || tModalAvatarFallback;
    tModalAvatarEl.onerror = function () {
      this.onerror = null;
      this.src = tModalAvatarFallback;
    };
  }
  ...
  document.getElementById("tModalContact").innerHTML =
    mField("Phone", t.phone, "tphone") +
    mField("Email", t.email, "temail") +
    mField("Emergency Contact", t.emergencyContact, "temergency") +
    mField("Course", t.course, "tcourse") +
    mFieldSel("Status", t.status, "tstatus", ["Active", "Inactive"]);
  document.getElementById("tModalProf").innerHTML =
    mFieldSel("Designation", t.designation, "tdesig", [
      "Assistant Professor",
      "Associate Professor",
      "Professor",
    ]) +
    mField("Qualification", t.qualification, "tqualification") +
    mField("Experience", t.experience, "texperience") +
    mField("Join Date", t.joinDate, "tjoinDate");
  let detail = null;
  try {
    detail = await apiJson(
      `/api/hod/teachers/${encodeURIComponent(t._id || t.id)}/detail`,
    );
  } catch (e) {
  ...
  document
    .querySelectorAll("#teacherModal .tchr-dyn-section")
    .forEach((el) => el.remove());
  let firstTSec = document.querySelector(
    "#teacherModal .modal-body .modal-section",
  );
  if (firstTSec) firstTSec.insertAdjacentHTML("beforebegin", lecHtml);
  document
    .querySelectorAll("#teacherModal .tchr-sched-section")
    .forEach((el) => el.remove());
  if (firstTSec)
    firstTSec.insertAdjacentHTML("beforebegin", teacherScheduleSectionHTML(t));
...
function updateTeacherLectureSubjects() {
  ...
  if (!sel) return;
  const subs =
    typeof getSubjObjects === "function" ? getSubjObjects(course, sem) : [];
...
async function renderTeacherScheduleList(t) {
  ...
  try {
    const data = await apiJson(
      `/api/hod/schedule?teacher=${encodeURIComponent(t._id || t.id)}`,
    );
    const docs = (data.docs || []).slice().sort((a, b) => {
      const di =
        TCHR_SCHED_DAYS.indexOf(a.day) - TCHR_SCHED_DAYS.indexOf(b.day);
      return di !== 0
        ? di
    ...
    if (!docs.length) {
      listEl.innerHTML =
        '<div class="sched-empty-day">No lectures scheduled for this teacher yet.</div>';
      return;
    }
...
async function saveTeacherLecture() {
  ...
  if (!subjectId) {
    showToast("Please select a subject from the Admin subject list.", true);
    return;
  }
  if (!_isLectureDurationValid(startTime, endTime)) {
    showToast("A lecture must be at least 30 minutes long.", true);
    return;
  }
  try {
...
function renderMarksModalContent() {
  ...
  let q = (document.getElementById("marksSubjectSearch")?.value || "")
    .trim()
    .toLowerCase();

  /* Look up saved marks for this student's course+sem+type */
  let marksData =
    ((savedMarks[s.course] || {})[s.sem] || {})[currentMarksType] || [];
  ...
  let total = 0,
    maxTotal = subjects.length * 30;
  let rows = subjects.map((sub) => {
    let m = studentEntry.subjects ? studentEntry.subjects[sub] : 0;
    m = m || 0;
    total += m;
          ...
          : pct >= 60
            ...
            : pct >= 50
              ...
              : pct >= 40
                ...
                : "F";
    let cls =
      pct >= 60 ? "badge-green" : pct >= 40 ? "badge-yellow" : "badge-red";
    return `<tr>
