...
function openRemoveStudentConfirm(id, name) {
  if (!confirm(`Move "${name}" to Removed? Their record and all data stay intact — you can bring them back into their same course/semester any time from the Removed section.`)) return;
  apiJson('/api/hod/students/remove', { method: 'POST', body: JSON.stringify({ ids: [id] }) })
    .then(() => { showToast('Student moved to Removed.'); refreshStudents().then(renderStudentList); })
...
function openDeleteStudentConfirm(id, name) {
  confirmDeleteModal({ itemLabel: 'student', name, onConfirm: () => {
    apiJson('/api/hod/students/' + id, { method: 'DELETE' })
      .then(() => { showToast('Student permanently deleted.'); refreshStudents().then(renderStudentList); })
      .catch(e => showToast(e.message || 'Failed', true));
  }});
}
...
async function addStudent(){
  ...
  if(!activeStuCourse){showToast('Select a course tab before adding a student.',true);return;}
  const emailRe=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const studentEmail=document.getElementById("newStuEmail").value.trim();
  if(!studentEmail){showToast('Student email is required.',true);return;}
  if(!emailRe.test(studentEmail)){showToast('Student email is not a valid email address.',true);return;}
  const parentEmail=document.getElementById("newStuParentEmail").value.trim();
  if(parentEmail && !emailRe.test(parentEmail)){showToast('Parent email is not a valid email address.',true);return;}
...
async function selectRemovedSection(){
  activeStuCourse='__REMOVED__';activeStuSem=null;activeStuAllSems=false;
  stuDeleteMode=false;stuPromoteMode=false;removedRecoverMode=false;
  renderStuCourseCards();
  document.getElementById("stuSemTabs").style.display="none";
  document.getElementById("stuContent").innerHTML='<div class="loading-msg">Loading removed students…</div>';
  try{
    const data = await apiJson('/api/hod/students?status=Removed');
    removedStudents = data.students || data.data?.students || [];
    renderRemovedList();
  }catch(e){
    document.getElementById("stuContent").innerHTML=`<div class="empty-state"><p>${e.message||'Failed to load'}</p></div>`;
  }
}
...
function selectAllRemoved(){
  document.querySelectorAll('#stuContent .chk-col.show input[type=checkbox]').forEach(c=>c.checked=true);
  updateRemovedSelectedCount();
}

function updateRemovedSelectedCount(){
  const n = document.querySelectorAll('#stuContent .chk-col.show input[type=checkbox]:checked').length;
...
function confirmRecoverSelected(){
  const ids = removedStudents
    ...
    .map(s => s.id);
  if (!ids.length) { showToast('Select at least one student to recover.', true); return; }
  const names = removedStudents.filter(s=>ids.includes(s.id)).map(s=>`${s.name} (${s.course} Sem ${s.sem})`).join('\n');
  if (!confirm(`Recover ${ids.length} student(s) back into their original course/semester?\n\n${names}`)) return;
  apiJson('/api/hod/students/recover', { method:'POST', body: JSON.stringify({ ids }) })
    .then(async () => {
