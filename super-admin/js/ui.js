...
const UI = {
  ...
  confirm({
    ...
    if (requireTypedName) {
      wrap.style.display = "";
      document.getElementById("confirm-type-name-label").textContent =
        `Type "${requireTypedName}" to confirm:`;
      input.value = "";
      btn.disabled = true;
      btn.style.opacity = ".5";
      input.oninput = () => {
  ...
  avatarInnerHtml(person, innerHtml) {
    if (!(person && person.avatar)) return innerHtml;
    const safeFallback = String(innerHtml)
      .replace(/'/g, "&#39;")
      .replace(/"/g, "&quot;");
    return `<img src="${person.avatar}" alt="" style="width:100%;height:100%;border-radius:50%;object-fit:cover" onerror='this.parentElement.innerHTML="${safeFallback}"'>`;
  },
  pct(part, total) {
    if (!total) return 0;
  ...
  roleBadge(role) {
    const map = {
      super_admin: { label: "Super Admin", cls: "badge--amber" },
      admin: { label: "Principal", cls: "badge--purple" },
      hod: { label: "Dept Admin", cls: "badge--blue" },
      co_hod: { label: "Co-Dept Admin", cls: "badge--info" },
  ...
  skeletonRows(cols, count = 5) {
    const cells = Array(cols)
      .fill(`<td><div class="skeleton-line w60"></div></td>`)
      .join("");
  ...
  skeletonCards(count = 4) {
    return Array(count)
      .fill(
        `<div class="stat-card skeleton-card"><div class="skeleton-line w40"></div><div class="skeleton-line w60 mt8"></div></div>`,
      )
      .join("");
  },
  logout() {
    fetch("/api/auth/logout", {
      method: "POST",
      headers: { Authorization: "Bearer " + localStorage.getItem("sal_token") },
    }).catch(() => {});
    localStorage.clear();
