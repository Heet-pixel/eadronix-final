...
const UI = {
  /* ── Avatar photo → initials fallback ─────────────────────
     Used as the onerror handler on every profile <img> across the
     portal. If the photo URL fails to load (network hiccup, storage
     issue, etc.) this swaps it for the plain initials avatar instead of
     a broken image / raw alt-text showing. */
  /* ── Avatar photo → initials fallback ─────────────────────
     Used as the onerror handler on every profile <img> across the
     portal. If the photo URL fails to load (network hiccup, storage
     issue, etc.) this swaps it for the plain initials avatar instead of
     a broken image / raw alt-text showing — without disturbing sibling
     elements like the "change photo" edit button. */
  avatarFallback(imgEl, initials) {
    if (!imgEl) return;
    const parent = imgEl.parentElement;
    if (parent && parent.children.length === 1) {
      // img is the sole content of its container — safe to just swap
      // the container's text, matching how the no-photo case renders.
      parent.textContent = initials || "?";
      return;
    }
    // img has siblings (e.g. an edit button) — replace only the img
    // itself, carrying over its sizing classes/inline styles.
